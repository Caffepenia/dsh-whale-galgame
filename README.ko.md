...
DSH 0.1.0-rc.7 이상에서는 "설정 → 플러그인 → 플러그인 구성"에 있는 이 플러그인 카드의 "DashScope 密钥" 항목에서 직접 입력할 수도 있습니다. key는 이 저장소가 아니라 DSH 자체 설정 문서에 저장되며, 비밀 필드로 선언되어 값이 Host를 떠나기 전에 제거되므로 브라우저에는 절대 전송되지 않습니다. 그래서 이 입력란은 저장된 key를 표시하지 않고 설정 여부만 알려 주며, 비워 둔 채 저장해도 기존 key는 유지됩니다.
