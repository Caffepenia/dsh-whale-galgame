...
DSH 0.1.0-rc.7 以降では「設定 → プラグイン → プラグイン設定」にある本プラグインのカードの「DashScope 密钥」欄から直接入力することもできます。key は本リポジトリではなく DSH 自身の設定ドキュメントに保存され、機密フィールドとして宣言されているため、値が Host を離れる前に取り除かれ、ブラウザーへは決して送信されません。そのため、この入力欄は保存済みの key を表示せず、設定済みかどうかだけを示します。空のまま保存しても既存の key は消えません。
