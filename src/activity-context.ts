...
const ACTIVITY_DEFINITIONS: readonly ActivityDefinition[] = [
  {
    id: 'code-debug',
    label: '代码调试',
    strong: /\bdebug(?:ging)?\b|\bbug\b|traceback|stack\s*trace|test(?:s|ing)?\s+(?:fail|error)|ci\s+(?:fail|error)|报错|错误|异常|崩溃|闪退|失败|修复|排查|诊断|没反应|不工作|无法运行|報錯|錯誤|異常|崩潰|閃退|失敗|修復|診斷|沒反應|無法運行|當掉|當機|掛掉|壞掉|沒作用|無法執行|跑不起來|跑不動|丟出?例外|拋出?例外|例外處理/i,
  ...
  },
  {
    ...
    label: '文学创作',
    strong: /小说|诗歌?|剧本|故事|散文|文学创作|续写|世界观|角色设定|剧情|fiction|novel|poem|screenplay|story|小說|詩歌?|劇本|文學創作|續寫|世界觀|角色設定|劇情|連載|人設/i,
  ...
  },
  {
    ...
    weak: /文档|论文|文章|报告|pdf|docx|材料|会议|文檔|論文|報告|會議|逐字稿|講義|資料/i,
  ...
  },
  {
    ...
    weak: /数据|指标|表格|趋势|均值|中位数|样本|數據|指標|趨勢|中位數|樣本|資料|欄位|報表/i,
  ...
  },
  {
    ...
    weak: /界面|视觉|图片|图像|美术|颜色|字体|组件样式|視覺|圖片|圖像|美術|顏色|字體|字型|組件樣式|元件樣式|介面|色票|圖層|圖檔/i,
  ...
  },
  {
    ...
    weak: /中文|英文|日文|韩文|措辞|术语|语言|韓文|措辭|術語|語言|用詞|語感|譯名|繁體|簡體/i,
  ...
  },
  {
    ...
    weak: /论文|来源|证据|引用|网页|仓库|资料|論文|來源|證據|網頁|倉庫|資料|出處|文獻|網站|儲存庫/i,
  ...
  },
  {
    id: 'document-writing',
    label: '文档写作',
    strong: /readme|文档|报告|论文|提案|方案书|申请书|说明书|稿件|撰写|润色|改写|documentation|report|manuscript|proposal|文檔|報告|論文|方案書|申請書|說明書|撰寫|潤色|改寫|企劃書|計畫書|手冊|潤稿|設計文件|技術文件|規格文件|系統文件|文件撰寫|撰寫文件/i,
    weak: /章节|段落|措辞|结构|标题|编辑|写|章節|措辭|結構|標題|編輯|寫/i,
  ...
  },
  {
    id: 'code-development',
    label: '代码开发',
    strong: /写代码|编程|开发|实现|重构|加功能|新功能|接口|前端|后端|组件|函数|类|typescript|javascript|python|react|vue|node|api\b|coding|implement|refactor|寫代碼|寫程式|編程|程式設計|開發|實現|實作|重構|後端|組件|元件|函數|函式|類別/i,
    weak: /代码|程序|仓库|构建|依赖|插件|模型|服务|代碼|程式|倉庫|儲存庫|構建|建置|依賴|相依|套件|外掛|服務|專案|介面/i,
  ...
  },
  {
    id: 'planning',
    label: '任务规划',
    strong: /计划|规划|路线图|roadmap|里程碑|拆分任务|排期|待办|todo|project\s+plan|計劃|計畫|規劃|路線圖|拆分任務|待辦|時程|甘特圖/i,
    weak: /步骤|优先级|下一步|方案|安排|步驟|優先級|優先順序|進度/i,
